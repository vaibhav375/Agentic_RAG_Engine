...
def grade_retrieval(store, query: str, contexts: list[RetrievedChunk], cfg) -> RetrievalGrade:
    ...
    # Mode-sensitive in principle — how far this gate can relax depends on the
    # critic behind it — but both modes currently measure best at the same value,
    # so there is one. See the config comment before changing it.
